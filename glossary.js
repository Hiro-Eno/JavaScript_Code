...
{target:`.forEach()`, content:`配列の各要素に対して指定された関数を実行するメソッド。<br>array.***(callbackFn);<br>array.***((element) => console.log(element));<br>const elements = document.getElementsByTagName('h1');<br>Array.from(elements).***(x => console.lg(x)); → HTMLのh1タグを順番にコンソールに表示`},
...
{target:`.startsWith()`, content:`文字列が引数で指定された文字列で始まるかを判定して true か false を返すメソッド。<br>***(searchString, endPosition);<br>const str = 'Saturday night plans';<br>str.***('Sat'); → true<br>str.***('Sat', 3); → false;`},
{target:`.endsWith()`, content:`文字列が引数で指定された文字列で終わるかを判定して true か false を返すメソッド。<br>***(searchString, endPosition);<br>const str = 'Saturday night plans';<br>str.***('plans'); → true<br>str.***('plans', 20); → true;`},
