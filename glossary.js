...
{target:`Array.from()`, content:`反復可能オブジェクトや配列風オブジェクトからシャローコピーされた、新しい Array インスタンスを生成する静的メソッド。<br>const array = [1, 2, 3];<br>const copyArray = ***.***(array);<br>copyArray.pop();<br>const newArray = ***.***(array, x => x * 2);<br>array→[1, 2, 3], copyArray→[1, 2], newArray→[2, 4, 6]<br>***.***('foo')→['f', 'o', 'o']`},
