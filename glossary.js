...
{target:`for`, content:`ループ構造を表現するためのキーワード。<br>指定された条件で繰り返し処理を行う。<br>*** (initialization; condition; afterthought) {<br>&emsp;statement<br>};<br>*** (const key in obj){<br>};<br>*** (const I of array){<br>};<br>*** (let i = 0; i < 9; i++) {<br>};`},
...
{target:`.includes()`, content:`特定の要素が配列に含まれているかどうかを true または false で返すメソッド。<br>array.***(searchElement, fromIndex);<br>const array = [1, 2, 3];<br>array.***(2); → true<br>★object.keys(object).***(key); → objectのキーにkeyが存在するか判別。'key in object'と同じ`},
...
{target:`.replace()`, content:`pattern に一致する文字列の一部またはすべてをreplacementで置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement);<br>'foof'.***('f', 'A'); → 'Aoof'<br>★'Abc 123!!456'.replace(/[^A-Za-z0-9\s]/g, ''); → 'Abc 123456'（大文字、小文字、数字、スペース以外の文字を削除）`},
