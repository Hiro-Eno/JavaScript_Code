...
{target:`Object.assign()`, content:`すべての列挙可能な自身のプロパティの値を、 1 つ以上のコピー元オブジェクトからコピー先オブジェクトにコピーするために使用するメソッド。ターゲットに空のオブジェクトを指定することで、新たなオブジェクトにsourceのオブジェクトをコピーできる。<br>***.***(target, ...sources);<br>copyObj = ***.***({}, source);`},
