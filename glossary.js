...
{target:`function`, content:`指定された引数を使用して関数を定義するためのキーワード。<br>*** name(param) {statements}<br>*** square(x){<br>return x * x;<br>}`},
{target:`return`, content:`関数の実行を終了して、関数の呼び出し元に返す値を指定するためのキーワード。<br>*** expression;<br>function square(x){<br>*** x * x;<br>}`},
...
{target:`.filter()`, content:`配列の要素をフィルタリングするためのメソッド。テストに合格した要素だけを抽出したシャローコピーの配列を作成する。<br>array.***(callbackFn)<br>const array = [12, 5, 8, 130, 44];<br>array.***((x) => x > 10)→[12, 130, 44]<br>array.***(callbackFn, thisArg)`},
{target:`.forEach()`, content:`配列の各要素に対して指定された関数を実行するメソッド。<br>array.***(callbackFn)<br>array.***((element) => console.log(element));<br>array.***(callbackFn, thisArg)`},
...
{target:`.map()`, content:`配列の各要素に対して指定された関数を適用するメソッド。新しい配列を返す。<br>array.***(callbackFn)<br>array.***((x) => x * 2)<br>array.***(callbackFn, thisArg)`},
...
{target:`.reduce()`, content:`配列のそれぞれの要素に対して、「縮小」コールバック関数を呼び出し、直前の要素での計算結果の返値を渡すメソッド。配列の全要素に対して縮小関数を実行した結果を返す。コールバックの初回実行時は初期値（指定の無い場合は0）が使用され、次の要素から反復処理が開始される。縮小関数に((a, b) => a + b)を宣言すると配列の合計値が返される;<br>array.***(callbackFn, initialValue)<br>const array = [1, 2, 3, 4];<br>const initialValue = 0;<br>const sumWithInitial = array.***((accumulator, currentValue) => accumulator + currentValue,  initialValue);<br>sumWithInitial→10<br>array.***((a, b) => a + b)→10<br>array.***((a, b) => a * b)→24<br>array.***((a, b)=> a - b, 5)→-5 `},
{target:`.reduceRight()`, content:`アキュームレーターと配列のそれぞれの値に対して（右から左へ）関数を適用して、単一の値を返すメソッド。左から右へ適用する場合は.reduce() を参照。<br>array.***(callbackFn, initialValue)<br>const array = [[0, 1], [2, 3], [4, 5],];<br>const result = array.***((accumulator, currentValue) =><br>accumulator.concat(currentValue),<br>);<br>result→[4, 5, 2, 3, 0, 1]`},
...
{target:`.slice()`, content:`元の文字列を変更せず、文字列の一部分を取り出し、それを新しい文字列として返すメソッド。<br>***(indexStart, indexEnd)<br>const str = 'The quick brown fox jumps over the lazy dog.';<br>str.slice(31)→'the lazy dog.', <br>str.slice(4, 19)→'quick brown fox'<br>str.slice(-4)→'dog.', str.slice(-9, -5)→'lazy'`},
...
{target:`switch`, content:`式を評価して、一連のcase節に対してその式の値を照合し、最初に値が一致したcase節の後の文を、break文に出会うまで実行。一致したcase の後にある文も同様に実行。default節には、 caseが式の値と一致しない場合にジャンプする。<br>*** (expression) {case value1: 文 case value2: 文 // … case valueN: 文 default: 文}<br>const foo = 0;<br>switch (foo) {<br>  case -1:<br>    break;<br>  case 0:<br>    console.log(0);<br>  case 1:<br>    console.log(1);<br>  case 2:<br>    console.log(2);<br>    break;<br>  default:<br>    console.log('default');<br>}→0, 1, 2が順次コンソールに表示`},
{target:`this`, content:`オブジェクト自体を指すキーワード。<br>***`},
{target:`throw`, content:`例外を発生させるためのキーワード。<br>*** expression;`},
{target:`try`, content:`例外を試みるためのブロック。<br>*** {tryStatements} catch (exceptionVar) {catchStatements} finally {finallyStatements}<br>*** {<br>  nonExistentFunction();<br>} catch (error) {<br>  console.error(error);<br>}→errorが表示`},
{target:`catch`, content:`try文で発生した例外を捕捉するためのブロック。<br>try {tryStatements} *** (exceptionVar) {catchStatements} finally {finallyStatements}<br>try {<br>  nonExistentFunction();<br>} *** (error) {<br>  console.error(error);<br>}→errorが表示`},
