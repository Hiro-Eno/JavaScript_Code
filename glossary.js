...
{target:`if`, content:`条件文を表現するためのキーワード。条件が真の場合にコードを実行。<br>*** (condition) {};<br><br>***(){<br>}else ***(){<br>} else {<br>};`},
...
{target:`for`, content:`ループ構造を表現するためのキーワード。指定された条件で繰り返し処理を行う。<br>*** (initialization; condition; afterthought) {<br>&emsp;statement<br>};<br>*** (const key in obj){<br>};<br>*** (const of array){<br>};<br>*** (let i = 0; i < 9; i++) {<br>};`},
{target:`while`, content:`ループ構造を表現するためのキーワード。指定された条件が真の場合にコードを実行し、条件が偽になるまで繰り返す。<br>*** (condition) {<br>&emsp;statement<br>};<br>let n = 0;<br>*** (n < 3) {<br>&emsp;n++;<br>};`},
...
{target:`.filter()`, content:`配列の要素をフィルタリングするためのメソッド。テストに合格した要素だけを抽出したシャローコピーの配列を作成する。使用するcallbackFnは引数に要素、インデックス、配列を受け取れる。<br>array.***(callbackFn);<br>const array = [12, 5, 8, 5, 130, 12, 44];<br>array.***((x) => x > 10); → [12, 130, 12, 44]<br><br>★array.***((elem, index, array) => array.indexOf(elem) === index); → [12, 5, 8, 130, 44]（ユニークな要素を抽出）`},
{target:`.forEach()`, content:`配列の各要素に対して指定された関数を実行するメソッド。<br>array.***(callbackFn);<br>array.***((element) => console.log(element));<br>const elements = document.getElementsByTagName('h1');<br><br>★Array.from(elements).***(x => console.lg(x)); → HTMLのh1タグを順番にコンソールに表示`},
...
{target:`.length`, content:`要素の数や文字数を調べるプロパティ。配列オブジェクトはこのプロパティを監視し、自動的にこの値を配列のコンテンツと同期させ、新しい値を超えた要素は削除される。また、length に現在の長さ以上の値を設定すると疎配列を作成する。これを利用すると連番を生成できる。<br>array.***; → 要素数<br>'abc'.***; → 3<br>const numbers = [];<br>numbers.*** = 3;<br>console.log(numbers); → [empty × 3]（疎配列）<br><br>★const range = (start, stop, step) =><br>&emsp;Array.from({ length: (stop - start) / step + 1 }, (_, i) => start + i * step); → 連番関数<br>range(0, 4, 1); → [0, 1, 2, 3, 4]<br>range(1, 10, 2); → [1, 3, 5, 7, 9]`},
...
{target:`.shift()`, content:`配列から最初の要素を取り除き、その要素を返すメソッド。配列の長さを変える。<br>const array = [1, 2, 3];<br>const first = array.***(); → array → [2, 3], first → 1`},
...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。既定のソート順は昇順で、要素を文字列に変換してから、UTF-16 コード単位の値の並びとして比較。元の配列を変更せずに配列内の要素をソートするには、toSorted() を使用。関数を使用する場合、aとbの2値を引数とし、例えば降順の場合、aが大きい場合は負の、aが小さい場合は正の、同じ場合は0を返すように関数を作る。<br>array.***();<br>array.***(compareFn(a, b));<br><br>★const func = (a, b) => {return b - a}; → 降順にソートするのに使用<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***(); → [1, 200, 40, 5, '700', '80', '9']<br>array.***(func); → ['700', 200, '80', 40, '9', 5, 1]<br>★array.***((a, b) => {return b - a})[0]; → 最大値<br>★array.***((a, b) => {return a - b})[0]; → 最小値`},
...
{target:`.split()`, content:`文字列を指定の区切りで分割するメソッド。('')で1文字ずつ分割。<br>'apple,orange,banana'.***(','); → ['apple', 'orange', 'banana']<br>'str'.***(''); → ['s', 't', 'r']<br><br>★words.***(' ').length; → 単語数をカウント<br>★word.***('').filter(x => ['a', 'i', 'u', 'e', 'o'].includes(x.toLowerCase())).length; → 母音数をカウント`},
...
{target:`.every()`, content:`配列のそれぞれの要素に対して、指定されたcallbackFn関数が偽値を返すまで一度ずつ呼び出すメソッド。 そのような要素が見つかった場合、every()は直ちにfalseを返し、配列の反復処理を中止する。そうでない場合、callbackFnがすべての要素に対して真値を返すと、every()はtrueを返す。<br>array.***(callbackFn);<br>const overTen = (element) => element > 10;<br>[12, 5, 8, 130, 44].***(overTen); → false<br>[12, 54, 18, 130, 44].***(overTen); → true`},
...
{target:`.toLocaleString()`, content:`配列の要素を表す文字列を返すメソッド。引数localesとoptionsを指定して呼び出し、実装で定義された区切り文字 (',') でその結果を連結したものを返す。<br>const array = [1, 'a', new Date('21 Dec 1997 14:12:00 UTC')];<br>const localeString = array.toLocaleString('en', { timeZone: 'UTC' }); → '1,a,12/21/1997, 2:12:00 PM'<br>const prices = ['￥7', 500, 8123, 12];<br>prices.***('ja-JP', { style: 'currency', currency: 'JPY'}); → '￥7,￥500,￥8,123,￥12' `},
{target:`.toSorted()`, content:`.sort() メソッドに対応するコピーメソッド。要素を昇順にソートした新しい配列を返す。<br>const values = [1, 10, 21, 2];<br>const sortedValues = values.***((a, b) => a - b); → sortedValues → [1, 2, 10, 21], values → [1, 10, 21, 2]`},
{target:`.toSpliced()`, content:`.splice() メソッドに対応するコピーメソッド。指定された位置の要素を除去（削除）したり置き換えたり（置換）した新しい配列を返す。<br>array.***(startIndex, deleteCount, item1, item2);<br>const months = ['Jan', 'Feb', 'May'];<br>const monthsRevised = months.***(1, 1, 'Mar', 'Apr'); → monthsRevised → ['Jan', 'Mar', 'Apr', 'May'], months → ['Jan', 'Feb', 'May']　元配列はそのまま`},
{target:`.toString()`, content:`指定された配列とその要素を表す文字列を返すメソッド。<br>const array = [1, 2, 'a', '1a'];<br>array.***(); → '1,2,a,1a'`},
{target:`Array()`, content:`配列オブジェクトを生成するコンストラクター。スプレッド構文と組合せることでn個のundefinedからなる新しい配列を作成できる。これと.forEach等を組合せることでn回の繰り返し作業が可能（空はスキップされるためスプレッド構文による展開が必要）。<br>***('りんご', 'バナナ'); → ['りんご', 'バナナ']<br><br>★***(2); → [空, 空]<br>★[...***(3)]; → [undefined, undefined, undefined]`},
{target:`Array.from()`, content:`反復可能オブジェクトや配列風オブジェクトからシャローコピーされた、新しいArrayインスタンスを生成する静的メソッド。HTML要素の処理に使える。<br>const array = [1, 2, 3];<br>const copyArray = ***.***(array);<br>copyArray.pop();<br>const newArray = ***.***(array, x => x * 2);<br>array → [1, 2, 3], copyArray → [1, 2], newArray → [2, 4, 6]<br>***.***('foo'); → ['f', 'o', 'o']<br><br>★const elements = document.getElementsByTagName('h1');<br>***.***(elements).forEach(x => console.lg(x)); → HTMLのh1タグを順番にコンソールに表示`},
...
{target:`.replace()`, content:`pattern に一致する文字列の一部またはすべてをreplacementで置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement);<br>'foof'.***('f', 'A'); → 'Aoof'`},
...
{target:`Math.sign()`, content:`引数として渡された数値の符号が正か負かを表す+/- 1を返す関数。渡された数が0であれば、+/- 0を返す。数値が正である場合、明示的な (+) は返さない。<br>***.***(3); → 1`},
...
{target:`in`, content:`オブジェクトが指定されたプロパティを持っているかをチェックする演算子。<br>prop *** object;<br>const car = { make: 'TOYOTA', model: 'Prius', year: 1997 };<br>'make' in car; → true<br>'TOYOTA' in car; → false`},
...
{target:`.getElementsByName()`, content:`文書内で指定したnameを持つ要素のNodeListコレクションを返すDocumentオブジェクト。elementsは、生きたNodeListコレクションで文書内で同じnameを持った新しい要素が追加されたり削除されたりすると自動的に更新される。IE10 以前では指定されたid属性を持つ要素も返すため注意が必要。<br>document.***();`},
{target:`.querySelector()`, content:`指定されたセレクターまたはセレクター群に一致する、文書内の最初のElementを返すDOMメソッド。一致するものが見つからない場合はnullを返す。<br>document.***(selectors);`},
{target:`.querySelectorAll()`, content:`指定されたCSS セレクターに一致する文書中の要素のリストを示す静的な（生きていない）NodeListを返すDOMメソッド。<br>document.***(selectors);`},
{target:`.forms`, content:`文書のすべてのformを列挙するHTMLCollectionオブジェクト。コレクションのそれぞれの項目は、単一のform要素を表すHTMLFormElement。<br>const collection = document.***;<br>collection.length; → collection 内のアイテム数を返す。<br>collection.item(1); → リスト内の指定されたindex(先頭はゼロ) 位置にある特定のノードを返す。<br>collection.namedItem('Id'); → nameで指定した文字列にID、または代替としてnameが一致するノードを返す。<br><br>const select = document.***[1];<br>const selectElement = document.***[1].elements[0];<br>document.***['nameのプロパティ名'].elements['nameのプロパティ名'];`},
...
{target:`.textContent`, content:`要素のテキストコンテンツを取得または設定するプロパティ。scriptとstyle要素を含む、すべての要素の中身を取得。ノード内のすべての要素を返す。値がHTMLとして解析されないので性能が良くなる。また、XSS攻撃を防ぐことができる。<br>element.***;`},
...
{target:`.addEventListener()`, content:`イベントリスナーを要素に追加するDOMメソッド。リスナーには関数を指定。<br>element.***(type, listener);<br>element.*** = (type, () => func(x));<br>element.*** = (type, () => {});<br><br>element.*** = (type, (event) => {})<br>eventの例: event.buttons, event.clientX, event.pageX, event.screenX, event.shiftKey etc`},
...
{target:`console.time()`, content:`長い処理がどれくらいかかるかを計測することができるタイマーを起動するメソッド。タイマーに固有の名前を付けると、10,000個までのタイマーを実行できる。同じ名前を引数としてconsole.timeEnd()を呼び出すと、タイマー開始からの経過時間がミリ秒単位で出力。<br>***.***('func');<br>console.timeEnd('func'); → 経過時間を出力`},
...
{target:`.then()`, content:`最大2つの引数として、 Promise が成功した場合と失敗した場合のコールバック関数を取るメソッド。<br>fetch('...').***(response => {...}); → Promise の成功時に実行される処理を定義`},
...
{target:`.setDate()`, content:`日付オブジェクトの日を設定のメソッド。<br>new Date(); date.***(15); → dateの日を15に設定`},
{target:`.setFullYear()`, content:`日付オブジェクトの年を設定のメソッド。<br>new Date(); date.***(2023); → dateの年を2023に設定`},
{target:`.setHours()`, content:`日付オブジェクトの時を設定のメソッド。<br>new Date(); date.***(12); → dateの時を12に設定`},
{target:`.setMilliseconds()`, content:`日付オブジェクトのミリ秒を設定のメソッド。<br>new Date(); date.***(500); → dateのミリ秒を500に設定`},
{target:`.setMinutes()`, content:`日付オブジェクトの分を設定のメソッド。<br>new Date(); date.***(30); → dateの分を30に設定`},
{target:`.setMonth()`, content:`日付オブジェクトの月を設定のメソッド。<br>new Date(); date.***(5); → dateの月を5に設定`},
{target:`.setSeconds()`, content:`日付オブジェクトの秒を設定のメソッド。<br>new Date(); date.***(45); → dateの秒を45に設定`},
{target:`.setTime()`, content:`日付オブジェクトにエポックからのミリ秒を設定のメソッド。<br>new Date(); date.***(1609459200000); → dateを指定のエポックミリ秒に設定`},
{target:`.setUTCDate()`, content:`UTC時刻の日を設定のメソッド。<br>new Date(); date.***(10); → dateのUTC日を10に設定`},
{target:`.setUTCFullYear()`, content:`UTC時刻の年を設定のメソッド。<br>new Date(); date.***(2023); → dateのUTC年を2023に設定`},
{target:`.setUTCHours()`, content:`UTC時刻の時を設定のメソッド。<br>new Date(); date.***(8); → dateのUTC時を8に設定`},
{target:`.setUTCMilliseconds()`, content:`UTC時刻のミリ秒を設定のメソッド。<br>new Date(); date.***(200); → dateのUTCミリ秒を200に設定`},
{target:`.setUTCMinutes()`, content:`UTC時刻の分を設定のメソッド。<br>new Date(); date.***(45); → dateのUTC分を45に設定`},
{target:`.setUTCMonth()`, content:`UTC時刻の月を設定のメソッド。<br>new Date(); date.***(5); → dateのUTC月を5に設定`},
{target:`.setUTCSeconds()`, content:`UTC時刻の秒を設定のメソッド。<br>new Date(); date.***(30); → dateのUTC秒を30に設定`},
{target:`.toDateString()`, content:`日付オブジェクトを文字列に変換のメソッド。<br>new Date(); date.***(); → dateを文字列に変換`},
{target:`.toISOString()`, content:`日付オブジェクトをISO形式の文字列に変換のメソッド。<br>new Date(); date.***(); → dateをISO形式の文字列に変換`},
{target:`.toJSON()`, content:`日付オブジェクトをJSON形式の文字列に変換のメソッド。<br>new Date(); date.***(); → dateをJSON形式の文字列に変換`},
{target:`.toTimeString()`, content:`日付オブジェクトの時刻部分を文字列に変換のメソッド。<br>new Date(); date.***(); → dateの時刻部分を文字列に変換`},
{target:`.toUTCString()`, content:`日付オブジェクトをUTC時刻の文字列に変換のメソッド。<br>new Date(); date.***(); → dateをUTC時刻の文字列に変換`},
