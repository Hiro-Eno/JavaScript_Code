...
{target:`while`, content:`ループ構造を表現するためのキーワード。指定された条件が真の場合にコードを実行し、条件が偽になるまで繰り返す。<br>*** (condition) {statement}<br>let n = 0;<br>while (n < 3) {n++;}`},
...
{target:`.concat()`, content:`二つの配列を結合するメソッド。新しい配列を返す。<br>array.***(array)<br>arr2 = arr2.***(arr)→arr2 = [...arr, ...arr2]（スプレッド構文と同様の働き）`},
{target:`.filter()`, content:`配列の要素をフィルタリングするためのメソッド。テストに合格した要素だけを抽出したシャローコピーの配列を作成する。<br>***(callbackFn)<br>array.***((word) => word.length > 6)<br>array.***(callbackFn, thisArg)`},
...
{target:`.length`, content:`要素の数や文字数を調べるプロパティ。配列オブジェクトはこのプロパティを監視し、自動的にこの値を配列のコンテンツと同期させ、新しい値を超えた要素は削除される。<br>array.***<br>'abc'.***→3`},
...
{target:`.pop()`, content:`配列の最後の要素を削除するメソッド。<br>array.***()<br>const array = [5, 12, 8, 130, 44];<br>array.***()→44<br>array→[5, 12, 8, 130]`},
...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。既定のソート順は昇順で、要素を文字列に変換してから、 UTF-16 コード単位の値の並びとして比較します。元の配列を変更せずに配列内の要素をソートするには、 toSorted() を使用。関数を使用する場合は、aとbの2値を引数とし、aが大きい場合は正の、aが小さい場合は負の、同じ場合は0を返すように関数を作る。<br>array.***()<br>array.***(compareFn(a, b))<br><br>const func = (a, b) => {return b - a}<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***()→[1, 200, 40, 5, '700', '80', '9']<br>array.***(func)→ ['700', 200, '80', 40, '9', 5, 1]`},
...
{target:`.every()`, content:`配列のそれぞれの要素に対して、指定された callbackFn 関数が偽値を返すまで一度ずつ呼び出すメソッド。 そのような要素が見つかった場合、 every() は直ちに false を返し、配列の反復処理を中止する。そうでない場合、 callbackFn がすべての要素に対して真値を返すと、 every() は true を返す。<br>array.***(callbackFn)<br>const isBigEnough = (element, index, array) => element >= 10;<br>[12, 5, 8, 130, 44].***(isBigEnough)→false<br>[12, 54, 18, 130, 44].***(isBigEnough)→true`},
...
{target:`.flat()`, content:`すべてのサブ配列の要素を指定した深さで再帰的に結合した新しい配列を生成するメソッド。<br>***(depth)<br>const array = [0, 1, [2, [3, [4, 5]]]];<br>array.***()→[0, 1, 2, [3, [4, 5]]]`},
...
{target:`.toReversed()`, content:`要素を逆順に並べた新しい配列を返すメソッド。<br>const newArray = array.***()`},
...
{target:`.toSpliced()`, content:`.splice() メソッドに対応するコピーメソッド。指定された位置の要素を除去したり置き換えたりした新しい配列を返す。<br>array.***(startIndex, deleteCount, item1, item2)<br>const months = ['Jan', 'Feb', 'May'];<br>const months1 = months.***(1, 1, 'Feb', 'Mar');<br>months1→['Jan', 'Feb', 'Mar', 'May'], months→['Jan', 'Feb', 'May']`},
...
{target:`Object.assign()`, content:`すべての列挙可能な自身のプロパティの値を、 1 つ以上のコピー元オブジェクトからコピー先オブジェクトにコピーするために使用するメソッド。ターゲットに空のオブジェクトを指定することで、新たなオブジェクトにsourceのオブジェクトのコピーを作れる。<br>***.***(target, ...sources);<br>const copyObj = ***.***({}, source)→新しいオブジェクトコピー`},
...
{target:`.includes()`, content:`1 つの文字列が別の文字列の中に含まれるかを判断し、true か false を返すメソッド。<br>***(searchString, position)<br>'Blue Whale'.toLowerCase().***('blue')→true`},
