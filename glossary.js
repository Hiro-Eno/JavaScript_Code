...
{target:`.replace()`, content:`pattern に一致する文字列の一部またはすべてをreplacementで置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement);<br>'foof'.***('f', 'A'); → 'Aoof'<br>★'Abc 123!!456'.***(/[^A-Za-z0-9\s]+/g, ' '); → 'Abc 123456' 大文字、小文字、数字、スペース以外の文字をスペースに変換<br>★text.***(/\s+/g, ' '); → 1つ以上連続する空白文字（改行、タブ、スペースなど）をスペースに変換`},
