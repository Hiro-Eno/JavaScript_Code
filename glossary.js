...
{target:`for`, content:`ループ構造を表現するためのキーワード。<br>指定された条件で繰り返し処理を行う。<br>*** (initialization; condition; afterthought) {<br>&emsp;statement<br>};<br>*** (const key in obj){<br>};<br>*** (const of array){<br>};<br>*** (let i = 0; i < 9; i++) {<br>};`},
...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。<br>既定のソート順は昇順で、要素を文字列に変換してから、UTF-16 コード単位の値の並びとして比較。元の配列を変更せずに配列内の要素をソートするには、toSorted() を使用。関数を使用する場合、aとbの2値を引数とし、例えば降順の場合、aが大きい場合は負の、aが小さい場合は正の、同じ場合は0を返すように関数を作る。<br>array.***();<br>array.***(compareFn(a, b));<br><br>★const func = (a, b) => {return b - a}; → 降順にソートするのに使用<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***(); → [1, 200, 40, 5, '700', '80', '9']<br>array.***(func); → ['700', 200, '80', 40, '9', 5, 1]<br>★array.***((a, b) => {return b - a})[0]; → 最大値<br>★array.***((a, b) => {return a - b})[0]; → 最小値`},
...
{target:`.some()`, content:`指定された関数で実装されているテストに、配列の中の少なくとも 1 つの要素が合格するかどうかを判定するメソッド。<br>配列の中で指定された関数が true を返す要素を見つけた場合は true を返し、そうでない場合は false を返す。それ以外の場合は false を返す。<br>array.***(callbackFn);<br>const array = [1, 2, 3, 4, 5];<br>array.***((x) => x % 2 === 0); → true`},
...
{target:`Array()`, content:`配列オブジェクトを生成するコンストラクター。<br>スプレッド構文と組合せることでn個のundefinedからなる新しい配列を作成できる。これと.forEach等を組合せることでn回の繰り返し作業が可能（空はスキップされるためスプレッド構文による展開が必要）。<br>***('りんご', 'バナナ'); → ['りんご', 'バナナ']<br><br>★***(2); → [空, 空]<br>★[...***(3)]; → [undefined, undefined, undefined]`},
...
{target:`Object.keys()`, content:`指定されたオブジェクトが持つプロパティのキーの配列を取得するメソッド。<br>通常のループで取得するのと同じ順序で返す。これによりオブジェクトのキーを対象とした配列のメソッドを利用可能になる。<br>***.***(obj);<br>const object = {a: 'somestring', b: 42, c: false,};<br>***.***(object); → ['a', 'b', 'c']`},
...
{target:`console.time()`, content:`長い処理がどれくらいかかるかを計測することができるタイマーを起動するメソッド。<br>タイマーに固有の名前を付けると、10,000個までのタイマーを実行できる。同じ名前を引数としてconsole.timeEnd()を呼び出すと、タイマー開始からの経過時間がミリ秒単位で出力。<br>***.***('func');<br>console.timeEnd('func'); → 経過時間を出力`},
