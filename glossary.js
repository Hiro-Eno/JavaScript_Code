...
{target:`for`, content:`ループ構造を表現するためのキーワード。<br>指定された条件で繰り返し処理を行う。<br>*** (initialization; condition; afterthought) {<br>&emsp;statement<br>};<br>*** (const key in obj){<br>};<br>*** (const elem of array){<br>};<br>*** (const idex in array){<br>};<br>*** (let i = 0; i < 9; i++) {<br>};`},
...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。<br>既定のソート順は昇順で、要素を文字列に変換してから、UTF-16 コード単位の値の並びとして比較。元の配列を変更せずに配列内の要素をソートするには、toSorted() を使用。関数を使用する場合、aとbの2値を引数とし、例えば降順の場合、aが大きい場合は負の、aが小さい場合は正の、同じ場合は0を返すように関数を作る。<br>array.***();<br>array.***(compareFn(a, b));<br><br>★const func = (a, b) => {return a - b}; → 昇順にソート<br>★const func = (a, b) => {return b - a}; → 降順にソート<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***(); → [1, 200, 40, 5, '700', '80', '9']<br>array.***(func); → ['700', 200, '80', 40, '9', 5, 1]<br>★array.***((a, b) => {return b - a})[0]; → 最大値<br>★array.***((a, b) => {return a - b})[0]; → 最小値`},
...
{target:`.replace()`, content:`pattern に一致する文字列の一部またはすべてをreplacementで置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement);<br>'foof'.***('f', 'A'); → 'Aoof'<br>★'Abc 123!!456'.***(/[^A-Za-z0-9\s]/g, ''); → 'Abc 123456'（大文字、小文字、数字、スペース以外の文字を削除）`},
