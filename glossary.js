...
`use strict`
const glossary = [
{target:`const`, content:`定数を宣言するためのキーワード。ブロックスコープを持つ。<br>*** name = value;<br>*** element = 'Apple'`},
{target:`let`, content:`変数を宣言するためのキーワード。ブロックスコープを持つ。<br>*** name = value;<br>*** element = 42`},
{target:`var`, content:`非推奨。変数を宣言するためのキーワード。再宣言が可能。<br>*** name = value;<br>*** element = '42'`},
{target:`if`, content:`条件文を表現するためのキーワード。条件が真の場合にコードを実行。<br>*** (condition) {}<br>***(){}else ***(){} else {}`},
{target:`else`, content:`if文の条件が偽の場合に実行されるブロックを指定するためのキーワード。<br>} *** {}<br>if(){}*** if(){} *** {}`},
{target:`else if`, content:`if文の条件が偽で、さらなる条件を検証するためのキーワード。<br>} *** *** (condition) {}<br>if(){}*** ***(){} else {}`},
{target:`function`, content:`指定された引数を使用して関数を定義するためのキーワード。<br>*** name(param) {statements}<br>*** square(x){<br>&emsp;return x * x;<br>}`},
{target:`return`, content:`関数の実行を終了して、関数の呼び出し元に返す値を指定するためのキーワード。<br>*** expression;<br>function square(x){<br>&emsp;*** x * x;<br>}`},
{target:`for`, content:`ループ構造を表現するためのキーワード。指定された条件で繰り返し処理を行う。<br>*** (initialization; condition; afterthought) {statement}<br>*** (const key in obj){}<br>*** (const of array){}<br>*** (let i = 0; i < 9; i++) {}`},
{target:`while`, content:`ループ構造を表現するためのキーワード。指定された条件が真の場合にコードを実行し、条件が偽になるまで繰り返す。<br>*** (condition) {statement}<br>let n = 0;<br>*** (n < 3) {n++;}`},
...
{target:`Function()`, content:`関数オブジェクトを生成するコンストラクター。コンストラクターを直接呼び出すと動的に関数を生成することができる。グローバルスコープで実行される関数のみを生成。<br>const sum = new ***('a', 'b', 'return a + b');<br>sum(2, 6) → 8`},
...
{target:`.concat()`, content:`二つの配列を結合するメソッド。新しい配列を返す。<br>array1.***(array2)<br>array3 = array1.***(array2) → array3 = [...array1, ...array2]（スプレッド構文と同様の働き）`},
...
{target:`.forEach()`, content:`配列の各要素に対して指定された関数を実行するメソッド。<br>array.***(callbackFn)<br>array.***((element) => console.log(element));<br><br>const elements = document.getElementsByTagName('h1');<br>Array.from(elements).***(x => console.lg(x)) → HTMLのh1タグを順番にコンソールに表示`},
{target:`.map()`, content:`配列の各要素に対して指定された関数を適用するメソッド。新しい配列を返す。オブジェクトの配列を受け取るときキーを受け取れる。<br>array.***(callbackFn)<br>array.***((x) => x * 2)<br>const objArray = [<br>&emsp;{ key: 'A', value: 1 },<br>&emsp;{ key: 'B', value: 2 },<br>&emsp;{ key: 'C', value: 3 },<br>];<br>const array = objArray.map(({ key, value }) => ({ [key]: value })); → [{ 'A': 1 }, { 'B': 2 }, { 'C': 3 }]`},
...
{target:`.shift()`, content:`配列から最初の要素を取り除き、その要素を返すメソッド。配列の長さを変える。<br>const array = [1, 2, 3];<br>const first = array.***();<br>array → [2, 3], first → 1`},
...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。既定のソート順は昇順で、要素を文字列に変換してから、 UTF-16 コード単位の値の並びとして比較。元の配列を変更せずに配列内の要素をソートするには、 toSorted() を使用。関数を使用する場合、aとbの2値を引数とし、例えば降順の場合、aが大きい場合は負の、aが小さい場合は正の、同じ場合は0を返すように関数を作る。<br>array.***()<br>array.***(compareFn(a, b))<br><br>const func = (a, b) => {return b - a}<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***() → [1, 200, 40, 5, '700', '80', '9']<br>array.***(func) → ['700', 200, '80', 40, '9', 5, 1]<br>array.***((a, b) => {return b - a})[0] → 最大値<br>array.***((a, b) => {return a - b})[0] → 最小値`},
{target:`.slice()`, content:`配列の一部を start から end （end は含まれない）までの範囲で、選択した新しい配列オブジェクトにシャローコピーして返すメソッド。 元の配列は変更されない。<br>array.***(start, end)<br>const array = [1, 2, 3, 4, 5];<br>array.***(2, 4) → [3, 4]<br>array.***() → シャローコピー、[...array]と同様`},
...
{target:`.split()`, content:`文字列を指定の区切りで分割するメソッド。('')で1文字ずつ分割。<br>'apple,orange,banana'.***(',') → ['apple', 'orange', 'banana']<br>'str'.***('') → ['s', 't', 'r']<br>words.***(' ').length → 単語数をカウント<br>word.***('').filter(x => ['a', 'i', 'u', 'e', 'o'].includes(x.toLowerCase())).length → 母音数をカウント`},
...
{target:`.every()`, content:`配列のそれぞれの要素に対して、指定された callbackFn 関数が偽値を返すまで一度ずつ呼び出すメソッド。 そのような要素が見つかった場合、 every() は直ちに false を返し、配列の反復処理を中止する。そうでない場合、 callbackFn がすべての要素に対して真値を返すと、 every() は true を返す。<br>array.***(callbackFn)<br>const overTen = (element) => element > 10;<br>[12, 5, 8, 130, 44].***(overTen) → false<br>[12, 54, 18, 130, 44].***(overTen) → true`},
...
{target:`.reduce()`, content:`配列のそれぞれの要素に対して、「縮小」コールバック関数を呼び出し、直前の要素での計算結果の返値を渡すメソッド。配列の全要素に対して縮小関数を実行した結果を返す。コールバックの初回実行時は初期値（指定の無い場合は0）が使用され、次の要素から反復処理が開始される。縮小関数に((a, b) => a + b)を宣言すると配列の合計値が返される。また、accumulatorに配列を利用する時はinitialValueに空配列を指定する。<br>array.***(callbackFn, initialValue)<br>const array = [1, 2, 3, 2, 4];<br>const initialValue = 0;<br>const sumWithInitial = array.***((accumulator, currentValue) => accumulator + currentValue,  initialValue);<br>sumWithInitial → 13<br>array.***((a, b) => a + b) → 12<br>array.***((a, b) => a * b) → 48<br>array.***((a, b)=> a - b, 5) → -7<br>array.***((accum, num) =>{<br>&emsp;accum.includes(num) ? null: accum.push(num);<br>&emsp;return accum;<br>}, []); → [1, 2, 3, 4]（ユニークな要素を抽出）<br>Array.from(word).reverse().***((a, b) => a + b) → 文字列を逆順<br>array.***((a, b) => [...a, ...b]); 多次元配列をフラット化<br>words.split(' ').***((a, b) => a[0].toUpperCase() + a.slice(1) + ' ' + b[0].toUpperCase() + b.slice(1)); → 各単語の最初の文字を大文字にする<br>array.***((a, b) => {a.includes(b) ? NaN: a.push(b); return a;}, []); → 配列内の重複する要素を削除する`},
{target:`.reduceRight()`, content:`アキュームレーターと配列のそれぞれの値に対して（右から左へ）関数を適用して、単一の値を返すメソッド。左から右へ適用する場合は.reduce() を参照。<br>array.***(callbackFn, initialValue)<br>const array = [[0, 1], [2, 3], [4, 5],];<br>const result = array.***((accumulator, currentValue) =><br>&emsp;accumulator.concat(currentValue);<br>);<br>result → [4, 5, 2, 3, 0, 1]`},
{target:`.some()`, content:`指定された関数で実装されているテストに、配列の中の少なくとも 1 つの要素が合格するかどうかを判定するメソッド。配列の中で指定された関数が true を返す要素を見つけた場合は true を返し、そうでない場合は false を返す。それ以外の場合は false を返す。<br>array.***(callbackFn)<br>const array = [1, 2, 3, 4, 5];<br>array.***((x) => x % 2 === 0) → true`},
{target:`.values()`, content:`配列の各インデックスの値を含む新しい配列イテレーターオブジェクトを返すメソッド。<br>array.***()`},
{target:`.with()`, content:`ブラケット記法に対応するコピーメソッド。指定された位置の要素を指定された値で置き換えた新しい配列を返す。<br>arrayInstance.***(index, value)<br>const array = [1, 2, 3, 4, 5];<br>array.***(2, 6) → [1, 2, 6, 4, 5], array → [1, 2, 3, 4, 5]`},
{target:`.toReversed()`, content:`要素を逆順に並べた新しい配列を返すメソッド。<br>const array = [1, 2, 3]<br>const newArray = array.***() → [3, 2, 1]`},
{target:`.toLocaleString()`, content:`配列の要素を表す文字列を返すメソッド。引数 locales と options を指定して呼び出し、実装で定義された区切り文字 (',') でその結果を連結したものを返す。<br>const array = [1, 'a', new Date('21 Dec 1997 14:12:00 UTC')];<br>const localeString = array.toLocaleString('en', { timeZone: 'UTC' }) → '1,a,12/21/1997, 2:12:00 PM'<br>const prices = ['￥7', 500, 8123, 12];<br>prices.***('ja-JP', { style: 'currency', currency: 'JPY'}); → '￥7,￥500,￥8,123,￥12' `},
...
{target:`.toSpliced()`, content:`.splice() メソッドに対応するコピーメソッド。指定された位置の要素を除去（削除）したり置き換えたり（置換）した新しい配列を返す。<br>array.***(startIndex, deleteCount, item1, item2)<br>const months = ['Jan', 'Feb', 'May'];<br>const monthsRevised = months.***(1, 1, 'Mar', 'Apr');<br>monthsRevised → ['Jan', 'Mar', 'Apr', 'May']<br> months → ['Jan', 'Feb', 'May']　元配列はそのまま`},
...
{target:`Array.from()`, content:`反復可能オブジェクトや配列風オブジェクトからシャローコピーされた、新しい Array インスタンスを生成する静的メソッド。HTML要素の処理に使える。<br>const array = [1, 2, 3];<br>const copyArray = ***.***(array);<br>copyArray.pop();<br>const newArray = ***.***(array, x => x * 2);<br>array → [1, 2, 3], copyArray → [1, 2], newArray → [2, 4, 6]<br>***.***('foo') → ['f', 'o', 'o']<br><br>const elements = document.getElementsByTagName('h1');<br>***.***(elements).forEach(x => console.lg(x)) → HTMLのh1タグを順番にコンソールに表示`},
...
{target:`Object.assign()`, content:`すべての列挙可能な自身のプロパティの値を、 1 つ以上のコピー元オブジェクトからコピー先オブジェクトにコピーするために使用するメソッド。変更されたコピー先オブジェクトを返す。ターゲットに空のオブジェクトを指定することで、新たなオブジェクトにオブジェクトのコピーを作れる。<br>***.***(target, ...sources);<br>const target = { a: 1, b: 2 };<br>const source = { b: 4, c: 5 };<br>const returnedTarget = ***.***(target, source);<br>const copyObj = ***.***({}, source)<br>target → {a: 1, b: 4, c: 5}, returnedTarget → {a: 1, b: 4, c: 5}, source → {b: 4, c: 5}<br>copyObj → {b: 4, c: 5}（新しいオブジェクトコピー）`},
{target:`Object.keys()`, content:`指定されたオブジェクトが持つプロパティのキーの配列を取得するメソッド。通常のループで取得するのと同じ順序で返す。これによりオブジェクトのキーを対象とした配列のメソッドを利用可能になる。<br>***.***(obj)<br>const object = {a: 'somestring', b: 42, c: false,};<br>***.***(object) → ['a', 'b', 'c']`},
{target:`Object.values()`, content:`指定されたオブジェクトが持つ列挙可能なプロパティの値を取得するメソッド。 for...in ループで提供される場合と同じ順序で配列にして返す。これによりオブジェクトの値を対象とした配列のメソッドを利用可能になる。<br>***.***(obj);<br>const object = {a: 'somestring', b: 42, c: false,};<br>***.***(object) → ['somestring', 42, false]`},
{target:`Object.groupBy()`, content:`指定されたコールバック関数によって返された文字列値に従って、指定された反復可能な要素をグループ化する静的メソッド。返されるオブジェクトには、グループごとに個別のプロパティがあり、グループ内の要素を含む配列が格納。<br>***.***(items, callbackFn)<br>const inventory = [<br>&emsp;{ name: 'asparagus', type: 'vegetables', quantity: 5 },<br>&emsp;{ name: 'bananas', type: 'fruit', quantity: 0 },<br>&emsp;{ name: 'goat', type: 'meat', quantity: 23 },<br>&emsp;{ name: 'cherries', type: 'fruit', quantity: 5 },<br>&emsp;{ name: 'fish', type: 'meat', quantity: 22 },<br>};<br>const result = ***.***(inventory, ({ type }) => type);<br>result → {vegetables: [{ name: 'asparagus', type: 'vegetables', quantity: 5 },],<br>&emsp;fruit: [{ name: 'bananas', type: 'fruit', quantity: 0 },<br>&emsp;&emsp;{ name: 'cherries', type: 'fruit', quantity: 5 }],<br>&emsp;meat: [{ name: 'goat', type: 'meat', quantity: 23 },<br>&emsp;&emsp;{ name: 'fish', type: 'meat', quantity: 22 }]}`},
...
{target:`.startsWith()`, content:`文字列が引数で指定された文字列で始まるかを判定して true か false を返すメソッド。<br>***(searchString, endPosition)<br>const str = 'Saturday night plans';<br>str.***('Sat') → true, str.***('Sat', 3) → false;`},
{target:`.endsWith()`, content:`文字列が引数で指定された文字列で終わるかを判定して true か false を返すメソッド。<br>***(searchString, endPosition)<br>const str = 'Saturday night plans';<br>str.***('plans') → true, str.***('plans', 20) → true;`},
{target:`.includes()`, content:`1 つの文字列が別の文字列の中に含まれるかを判断し、true か false を返すメソッド。<br>***(searchString, position)<br>'Blue Whale'.***('Blue') → true<br>'Blue Whale'.***('Blue', 1) → false<br>'Blue Whale'.***('blue') → false<br>'Blue Whale'.toLowerCase().***('blue') → true`},
{target:`.indexOf()`, content:`この文字列を検索し、指定した部分文字列が最初に出現するインデックスを返すメソッド。<br>***(searchString, position)<br>'Blue Whale'.***('Blue') → 0<br>'Blue Whale'.***('Whale', 0) → 5<br>'Blue Whale'.indexOf('Whale', 7) → -1<br>'Blue Whale'.indexOf('', 11) → 10`},
{target:`.padEnd()`, content:`結果の文字列が指定した長さになるように、現在の文字列を他の文字列で（必要に応じて繰り返して）延長するメソッド。延長は、現在の文字列の末尾から適用される。<br>***(targetLength, padString)<br>const str = 'Bread';<br>str.***(10, '.')) → 'Bread.....'`},
{target:`.padStart()`, content:`結果の文字列が指定した長さになるように、現在の文字列を他の文字列で（必要に応じて繰り返して）延長するメソッド。延長は、現在の文字列の最初から適用される。<br>***(targetLength, padString)<br>const str = 'Bread';<br>str.***(10, '.')) → '.....Bread'`},
{target:`.repeat()`, content:`呼び出し元の文字列を指定した数だけコピーして結合した新しい文字列を構築して返すメソッド。<br>const mood = 'Happy! ';<br>***.repeat(3) → 'Happy! Happy! Happy! '`},
{target:`.replace()`, content:`pattern に一致する文字列の一部またはすべてを replacement で置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement)<br>'foof'.***('f', 'A'); → 'Aoof'`},
{target:`.replaceAll()`, content:`pattern に一致したすべての文字列を replacement で置き換えた新しい文字列を返すメソッド。<br>***(pattern, replacement)<br>'foof'.***('f', 'A'); → 'AooA'`},
{target:`.search()`, content:`正規表現とこの 文字列オブジェクトが一致するかどうかを調べるメソッド。<br>***(regexp)<br>'hey JudE'.***(/[A-Z]/) → 4<br>'hey JudE'['hey JudE'.***(/[A-Z]/)] → 'J'`},
...
{target:`.substring()`, content:`文字列の一部、開始インデックスから終了インデックスの手前まで、または終了インデックスが指定されなかった場合は文字列の最後までを返すメソッド。<br>***(indexStart, indexEnd)<br>const str = 'Mozilla';<br>str.***(1, 3) → 'oz', str.***(2) → 'zilla'`},
...
{target:`Math.max()`, content:`入力引数として与えられた 0 個以上の数値のうち最大の数を返す関数。<br>***.***(1, 3, 2) → 3<br>let arr = [1, 2, 3]<br>***.***(...arr) → 3<br>***.***.apply(null, arr) → 3`},
{target:`Math.min()`, content:`引数で渡されたもののうち最小の値を返す関数。<br>***.***(2, 3, 1) → 1<br>let arr = [1, 2, 3]<br>***.***(…arr) → 1<br>***.***.apply(null, arr) → 1`},
...
{target:`.getElementsByTagName()`, content:`指定されたタグ名を持つ要素の HTMLCollection を返すDOMメソッド。<br>document.***()`},
{target:`.getElementById()`, content:`id プロパティが指定された文字列に一致する要素を表す Element オブジェクトを返すDOMメソッド。<br>document.***()`},
{target:`.getElementsByClassName()`, content:`指定されたクラス名をすべて持つすべての子要素の配列風オブジェクトを返すDOMメソッド。見つかった要素は生きたHTMLCollection。<br>document.***()`},
{target:`.getElementsByName()`, content:`文書内で指定した name を持つ要素の NodeList コレクションを返すDocumentオブジェクト。elements は、生きたNodeList コレクションで文書内で同じ name を持った新しい要素が追加されたり削除されたりすると自動的に更新される。IE10 以前では指定された id 属性を持つ要素も返すため注意が必要。<br>document.***()`},
{target:`.querySelector()`, content:`指定されたセレクターまたはセレクター群に一致する、文書内の最初の Element を返すDOMメソッド。一致するものが見つからない場合は null を返す。<br>document.***(selectors)`},
...
{target:`.onclick`, content:`クリック時の動作のDOMメソッド。<br>element.*** = func<br>element.*** = () => func(x)<br>element.*** = () => {}<br><br>element.*** = (event) => {}<br>event: event.buttons, event.clientX, event.pageX, event.screenX, event.shiftKey etc`},
...
{target:`setTimeout()`, content:`一定時間後に指定された関数を実行するDOMメソッド。<br>***(code, delay)<br>***(() => {<br>&emsp;&emsp;console.log('Delayed for 1 second.');<br>&emsp;}, '1000');<br>***(() => { console.log('Hi!'); }, 1000); → 1秒後に 'Hi!' をログに表示`},
...
{target:`.addEventListener()`, content:`イベントリスナーを要素に追加するDOMメソッド。リスナーには関数を指定。<br>element.***(type, listener)<br>element.*** = (type, () => func(x))<br>element.*** = (type, () => {})<br><br>element.*** = (type, (event) => {})<br>event: event.buttons, event.clientX, event.pageX, event.screenX, event.shiftKey etc`},
...
{target:`DOMContentLoaded`, content:`HTML の初期文書が完全に読み込まれ解釈された時点で発生するイベント。<br>document.addEventListener('***', (event) => {console.log('loaded'); }); → DOMが完全に読み込まれたときにログ'loaded'をコンソールに出力`},
{target:`.location`, content:`現在のURL。代入で新しいURLへ移行するプロパティ。<br>document.***`},
...
{target:`classList`, content:`要素の class 属性を返すプロパティ。これを使用してクラスリストを操作できる。<br>element.***.add('newClass') → elementにnewClassクラスを追加`},
...
{target:`console.time()`, content:`長い処理がどれくらいかかるかを計測することができるタイマーを起動するメソッド。タイマーに固有の名前を付けると、 10,000 個までのタイマーを実行できる。同じ名前を引数として console.timeEnd() を呼び出すと、タイマー開始からの経過時間がミリ秒単位で出力。<br>***.***('func')<br>console.timeEnd('func') → 経過時間を出力`},
{target:`console.timeEnd()`, content:`console.time() の呼び出しで開始したタイマーを停止するメソッド。停止させると、経過時間が自動的にウェブコンソールに停止した時刻を示すインジケーターと共に表示。<br>console.time('func')<br>***.***('func') → 経過時間を出力`},
{target:`Date.now()`, content:`UTC (協定世界時) での 1970 年 1 月 1 日 0 時 0 分 0 秒 から現在までの経過時間をミリ秒単位で返すメソッド。<br>startTime = ***.***();<br>endTime = ***.***();<br>endTime - startTime → 処理時間`},
...
{target:`.apply()`, content:` this 値を指定して関数を呼び出し、 arguments は配列 (または配列風オブジェクト) として提供するメソッド。<br>func.***(context, argsArray); → func を指定した context で実行<br>func(...arr) → func.***(null, arr)（スプレッド構文と同様）<br>Math.max.***(null, arr)`},
