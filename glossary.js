...
{target:`.sort()`, content:`配列の要素をソートし、ソートされた同じ配列の参照を返すメソッド。既定のソート順は昇順で、要素を文字列に変換してから、 UTF-16 コード単位の値の並びとして比較。元の配列を変更せずに配列内の要素をソートするには、 toSorted() を使用。関数を使用する場合、aとbの2値を引数とし、例えば降順の場合、aが大きい場合は負の、aが小さい場合は正の、同じ場合は0を返すように関数を作る。<br>array.***()<br>array.***(compareFn(a, b))<br><br>const func = (a, b) => {return b - a}<br>const array = ['80', '9', '700', 40, 1, 5, 200];<br>array.***()→[1, 200, 40, 5, '700', '80', '9']<br>array.***(func)→ ['700', 200, '80', 40, '9', 5, 1]`},
...
{target:`.splice()`, content:`配列の既存の要素を取り除いたり、置き換えたり、新しい要素を追加するメソッド<br>array.***(start, deleteCount, item1)<br>const months = ['Jan', 'March', 'April', 'June'];<br>months.***(1, 0, 'Feb')→['Jan', 'Feb', 'March', 'April', 'June']`},
...
{target:`.toSpliced()`, content:`.splice() メソッドに対応するコピーメソッド。指定された位置の要素を除去したり置き換えたりした新しい配列を返す。<br>array.***(startIndex, deleteCount, item1, item2)<br>const months = ['Jan', 'Feb', 'May'];<br>const monthsRevised = months.***(1, 1, 'Mar', 'Apr');<br>monthsRevised→['Jan', 'Mar', 'Apr', 'May'], months→['Jan', 'Feb', 'May']`},
...
{target:`Object.keys()`, content:`指定されたオブジェクトが持つプロパティの 名前の配列を、通常のループで取得するのと同じ順序で返すメソッド。<br>***.***(obj)<br>const object = {a: 'somestring', b: 42, c: false,};<br>***.***(object)→['a', 'b', 'c']`},
{target:`Object.values()`, content:`指定されたオブジェクトが持つ列挙可能なプロパティの値を、 for...in ループで提供される場合と同じ順序で配列にして返すメソッド。<br>***.***(obj);<br>const object = {a: 'somestring', b: 42, c: false,};<br>***.***(object)→['somestring', 42, false]`},
...
{target:`class`, content:`この宣言によりプロトタイプベースの継承を使って、指定された名前の新しいクラスを作成。<br>*** name [extends otherName] {'クラス本体'}<br><br>*** ClassName {<br>&emsp;constructor(name) {<br>&emsp;&emsp;this.name = name;<br>&emsp;}<br>&emsp;firstProperty = 'Type';<br>&emsp;secondProperty = 10;<br>&emsp;firstMethod() {<br>&emsp;&emsp;console.log(this.name, this.secondProperty);<br>&emsp;}<br>}<br><br>const instance = new ClassName('example');<br>instance.name → 'example'<br>instance.firstProperty → 'type'<br>instance.firstMethod() → コンソールに'example'と'10'が表示`},
...
{target:`new`, content:`インスタンスを生成するための演算子。<br>class ClassName {<br>&emsp;constructor(name) {<br>&emsp;&emsp;this.name = name;<br>&emsp;}<br>&emsp;firstProperty = 'Type';<br>&emsp;secondProperty = 10;<br>&emsp;firstMethod() {<br>&emsp;&emsp;console.log(this.name, this.secondProperty);<br>&emsp;}<br>}<br>const newClass = *** ClassName('newClass');`},
