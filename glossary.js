...
{target:`.onclick`, content:`クリック時の動作のDOMメソッド。<br>element.*** = func<br>element.*** = () => func(x)<br>element.*** = () => {}<br><br>element.*** = (event) => {}<br>event: MouseEvent.buttons, MouseEvent.clientX, MouseEvent.pageX, MouseEvent.screenX, MouseEvent.shiftKey etc`},
...
{target:`.addEventListener()`, content:`イベントリスナーを要素に追加するDOMメソッド。リスナーには関数を指定。<br>element.***(type, listener)<br>element.*** = (type, () => func(x))<br>element.*** = (type, () => {})<br><br>element.*** = (type, (event) => {})<br>event: MouseEvent.buttons, MouseEvent.clientX, MouseEvent.pageX, MouseEvent.screenX, MouseEvent.shiftKey etc`},
